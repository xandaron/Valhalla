...
foreign lib {
	...
	createGlobalSession :: proc(api_version: Int, out_global_session: ^^Global_Session) -> Result ---

	// Create a global session, configured via GlobalSessionDesc (search paths,
	// GLSL support, minimum language version, ...).
	createGlobalSession2 :: proc(desc: ^Global_Session_Desc, out_global_session: ^^Global_Session) -> Result ---

	// Create a global session without setting up the core module. The core
	// module can then be loaded via Global_Session.load_core_module/compile_core_module.
	// EXPERIMENTAL.
	createGlobalSessionWithoutCoreModule :: proc(api_version: Int, out_global_session: ^^Global_Session) -> Result ---

	// Create a blob that copies `size` bytes from `data`. Caller owns the
	// returned reference and must Release() it.
	createBlob :: proc(data: rawptr, size: uint) -> ^Blob ---
...
// Create a global session with the built-in core module.
create_global_session :: proc() -> (global_session: ^Global_Session, ok: bool) {
...
// GLSL support, minimum language version, ...).
create_global_session2 :: proc(desc: ^Global_Session_Desc) -> (global_session: ^Global_Session, ok: bool) {
...
// module can then be loaded via Global_Session.load_core_module/compile_core_module.
// EXPERIMENTAL.
create_global_session_without_core_module :: proc() -> (global_session: ^Global_Session, ok: bool) {
