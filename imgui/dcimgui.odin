...
foreign lib {
	...
	@(link_name="ImGui_CreateContext") CreateContext     :: proc(shared_font_atlas: ^FontAtlas = nil) -> ^GuiContext ---
	@(link_name="ImGui_DestroyContext") DestroyContext    :: proc(ctx: ^GuiContext = nil) --- // NULL = destroy current context
	@(link_name="ImGui_GetCurrentContext") GetCurrentContext :: proc() -> ^GuiContext ---
	@(link_name="ImGui_SetCurrentContext") SetCurrentContext :: proc(ctx: ^GuiContext) ---

	// Main
	@(link_name="ImGui_GetIO") GetIO         :: proc() -> ^GuiIO ---         // access the ImGuiIO structure (mouse/keyboard/gamepad inputs, time, various configuration options/flags)
	@(link_name="ImGui_GetPlatformIO") GetPlatformIO :: proc() -> ^GuiPlatformIO --- // access the ImGuiPlatformIO structure (mostly hooks/functions to connect to platform/renderer and OS Clipboard, IME etc.)
	@(link_name="ImGui_GetStyle") GetStyle      :: proc() -> ^GuiStyle ---      // access the Style structure (colors, sizes). Always use PushStyleColor(), PushStyleVar() to modify style mid-frame!
	...
	@(link_name="ImGui_ShowStyleEditor") ShowStyleEditor         :: proc(ref: ^GuiStyle = nil) --- // add style editor block (not a window). you can pass in a reference ImGuiStyle structure to compare to, revert to and save to (else it uses the default style)
	...
	// Styles
	@(link_name="ImGui_StyleColorsDark") StyleColorsDark    :: proc(dst: ^GuiStyle = nil) --- // new, recommended style (default)
	@(link_name="ImGui_StyleColorsLight") StyleColorsLight   :: proc(dst: ^GuiStyle = nil) --- // best used with borders and a custom, thicker font
	@(link_name="ImGui_StyleColorsClassic") StyleColorsClassic :: proc(dst: ^GuiStyle = nil) --- // classic imgui style
	...
	@(link_name="ImGui_Begin") Begin :: proc(name: cstring, p_open: ^bool = nil, flags: GuiWindowFlags = {}) -> bool ---
	...
	//    such as BeginMenu/EndMenu, BeginPopup/EndPopup, etc. where the EndXXX call should only be called if the corresponding
	//    BeginXXX function returned true. Begin and BeginChild are the only odd ones out. Will be fixed in a future update.]
	@(link_name="ImGui_BeginChild") BeginChild   :: proc(str_id: cstring, size: Vec2 = {0, 0}, child_flags: GuiChildFlags = {}, window_flags: GuiWindowFlags = {}) -> bool ---
	@(link_name="ImGui_BeginChildID") BeginChildID :: proc(id: GuiID, size: Vec2 = {0, 0}, child_flags: GuiChildFlags = {}, window_flags: GuiWindowFlags = {}) -> bool ---
	...
	@(link_name="ImGui_IsWindowFocused") IsWindowFocused   :: proc(flags: GuiFocusedFlags = {}) -> bool --- // is current window focused? or its root/child, depending on flags. see flags for options.
	@(link_name="ImGui_IsWindowHovered") IsWindowHovered   :: proc(flags: GuiHoveredFlags = {}) -> bool --- // is current window hovered and hoverable (e.g. not blocked by a popup/modal)? See ImGuiHoveredFlags_ for options. IMPORTANT: If you are trying to check whether your mouse should be dispatched to Dear ImGui or to your underlying app, you should not use this function! Use the 'io.WantCaptureMouse' boolean for that! Refer to FAQ entry "How can I tell whether to dispatch mouse/keyboard to Dear ImGui or my application?" for details.
	...
	@(link_name="ImGui_GetWindowViewport") GetWindowViewport :: proc() -> ^GuiViewport ---                         // get viewport currently associated to the current window.

	// Window manipulation
	// - Prefer using SetNextXXX functions (before Begin) rather that SetXXX functions (after Begin).
	@(link_name="ImGui_SetNextWindowPosEx") SetNextWindowPos           :: proc(pos: Vec2, cond: GuiCond = .None, pivot: Vec2 = {0, 0}) --- // set next window position. call before Begin(). use pivot=(0.5f,0.5f) to center on given point, etc.
	@(link_name="ImGui_SetNextWindowSize") SetNextWindowSize            :: proc(size: Vec2, cond: GuiCond = .None) ---      // set next window size. set axis to 0.0f to force an auto-fit on this axis. call before Begin()
	@(link_name="ImGui_SetNextWindowSizeConstraints") SetNextWindowSizeConstraints :: proc(size_min: Vec2, size_max: Vec2, custom_callback: GuiSizeCallback = nil, custom_callback_data: rawptr = nil) --- // set next window size limits. use 0.0f or FLT_MAX if you don't want limits. Use -1 for both min and max of same axis to preserve current size (which itself is a constraint). Use callback to apply non-trivial programmatic constraints.
	...
	@(link_name="ImGui_SetNextWindowCollapsed") SetNextWindowCollapsed       :: proc(collapsed: bool, cond: GuiCond = .None) --- // set next window collapsed state. call before Begin()
	...
	@(link_name="ImGui_SetNextWindowViewport") SetNextWindowViewport        :: proc(viewport_id: GuiID) ---                       // set next window viewport
	@(link_name="ImGui_SetWindowPos") SetWindowPos                 :: proc(pos: Vec2, cond: GuiCond = .None) ---       // (not recommended) set current window position - call within Begin()/End(). prefer using SetNextWindowPos(), as this may incur tearing and side-effects.
	@(link_name="ImGui_SetWindowSize") SetWindowSize                :: proc(size: Vec2, cond: GuiCond = .None) ---      // (not recommended) set current window size - call within Begin()/End(). set to ImVec2(0, 0) to force an auto-fit. prefer using SetNextWindowSize(), as this may incur tearing and minor side-effects.
	@(link_name="ImGui_SetWindowCollapsed") SetWindowCollapsed           :: proc(collapsed: bool, cond: GuiCond = .None) --- // (not recommended) set current window collapsed state. prefer using SetNextWindowCollapsed().
	...
	@(link_name="ImGui_SetWindowPosStr") SetWindowPosStr              :: proc(name: cstring, pos: Vec2, cond: GuiCond = .None) --- // set named window position.
	@(link_name="ImGui_SetWindowSizeStr") SetWindowSizeStr             :: proc(name: cstring, size: Vec2, cond: GuiCond = .None) --- // set named window size. set axis to 0.0f to force an auto-fit on this axis.
	@(link_name="ImGui_SetWindowCollapsedStr") SetWindowCollapsedStr        :: proc(name: cstring, collapsed: bool, cond: GuiCond = .None) --- // set named window collapsed state
	...
	@(link_name="ImGui_PushStyleColor") PushStyleColor       :: proc(idx: GuiCol, col: u32) ---               // modify a style color. always use this if you modify the style after NewFrame().
	@(link_name="ImGui_PushStyleColorImVec4") PushStyleColorImVec4 :: proc(idx: GuiCol, col: Vec4) ---
	@(link_name="ImGui_PopStyleColorEx") PopStyleColor      :: proc(count: i32 = 1) ---
	@(link_name="ImGui_PushStyleVar") PushStyleVar         :: proc(idx: GuiStyleVar, val: f32) ---          // modify a style float variable. always use this if you modify the style after NewFrame()!
	@(link_name="ImGui_PushStyleVarImVec2") PushStyleVarImVec2   :: proc(idx: GuiStyleVar, val: Vec2) ---         // modify a style ImVec2 variable. "
	@(link_name="ImGui_PushStyleVarX") PushStyleVarX        :: proc(idx: GuiStyleVar, val_x: f32) ---        // modify X component of a style ImVec2 variable. "
	@(link_name="ImGui_PushStyleVarY") PushStyleVarY        :: proc(idx: GuiStyleVar, val_y: f32) ---        // modify Y component of a style ImVec2 variable. "
	@(link_name="ImGui_PopStyleVarEx") PopStyleVar        :: proc(count: i32 = 1) ---
	@(link_name="ImGui_PushItemFlag") PushItemFlag         :: proc(option: GuiItemFlags, enabled: bool) --- // modify specified shared item flag, e.g. PushItemFlag(ImGuiItemFlags_NoTabStop, true)
	...
	@(link_name="ImGui_PushTextWrapPos") PushTextWrapPos  :: proc(wrap_local_pos_x: f32 = 0) --- // push word-wrapping position for Text*() commands. < 0.0f: no wrapping; 0.0f: wrap to end of window (or column); > 0.0f: wrap at 'wrap_pos_x' position in window local space
	...
	@(link_name="ImGui_GetColorU32Ex") GetColorU32          :: proc(idx: GuiCol, alpha_mul: f32 = 1) -> u32 --- // retrieve given style color with style alpha applied and optional extra alpha multiplier, packed as a 32-bit value suitable for ImDrawList
	...
	@(link_name="ImGui_GetColorU32ImU32Ex") GetColorU32ImU32     :: proc(col: u32, alpha_mul: f32 = 1) -> u32 ---    // retrieve given color with style alpha applied, packed as a 32-bit value suitable for ImDrawList
	@(link_name="ImGui_GetStyleColorVec4") GetStyleColorVec4      :: proc(idx: GuiCol) -> ^Vec4 ---                            // retrieve style color as stored in ImGuiStyle structure. use to feed back into PushStyleColor(), otherwise use GetColorU32() to get style color with style alpha baked in.
	...
	@(link_name="ImGui_SameLineEx") SameLine                   :: proc(offset_from_start_x: f32 = 0, spacing: f32 = -1) --- // call between widgets or groups to layout them horizontally. X position given in window coordinates.
	...
	@(link_name="ImGui_IndentEx") Indent                     :: proc(indent_w: f32 = 0) --- // move content position toward the right, by indent_w, or style.IndentSpacing if indent_w <= 0
	@(link_name="ImGui_UnindentEx") Unindent                   :: proc(indent_w: f32 = 0) --- // move content position back to the left, by indent_w, or style.IndentSpacing if indent_w <= 0
	...
	@(link_name="ImGui_GetID") GetID     :: proc(str_id: cstring) -> GuiID ---                   // calculate unique ID (hash of whole ID stack + given parameter). e.g. if you want to query into ImGuiStorage yourself
	@(link_name="ImGui_GetIDStr") GetIDStr  :: proc(str_id_begin: cstring, str_id_end: cstring) -> GuiID ---
	@(link_name="ImGui_GetIDPtr") GetIDPtr  :: proc(ptr_id: rawptr) -> GuiID ---
	@(link_name="ImGui_GetIDInt") GetIDInt  :: proc(int_id: i32) -> GuiID ---
	...
	@(link_name="ImGui_Text") Text              :: proc(fmt: cstring, #c_vararg _: ..any) ---                 // formatted text
	@(link_name="ImGui_TextV") TextV             :: proc(fmt: cstring, args: c.va_list) ---
	@(link_name="ImGui_TextColored") TextColored       :: proc(col: Vec4, fmt: cstring, #c_vararg _: ..any) ---      // shortcut for PushStyleColor(ImGuiCol_Text, col); Text(fmt, ...); PopStyleColor();
	@(link_name="ImGui_TextColoredV") TextColoredV      :: proc(col: Vec4, fmt: cstring, args: c.va_list) ---
	@(link_name="ImGui_TextDisabled") TextDisabled      :: proc(fmt: cstring, #c_vararg _: ..any) ---                 // shortcut for PushStyleColor(ImGuiCol_Text, style.Colors[ImGuiCol_TextDisabled]); Text(fmt, ...); PopStyleColor();
	@(link_name="ImGui_TextDisabledV") TextDisabledV     :: proc(fmt: cstring, args: c.va_list) ---
	@(link_name="ImGui_TextWrapped") TextWrapped       :: proc(fmt: cstring, #c_vararg _: ..any) ---                 // shortcut for PushTextWrapPos(0.0f); Text(fmt, ...); PopTextWrapPos();. Note that this won't work on an auto-resizing window if there's no other widgets to extend the window width, yoy may need to set a size using SetNextWindowSize().
	@(link_name="ImGui_TextWrappedV") TextWrappedV      :: proc(fmt: cstring, args: c.va_list) ---
	@(link_name="ImGui_LabelText") LabelText         :: proc(label: cstring, fmt: cstring, #c_vararg _: ..any) --- // display text+label aligned the same way as value+label widgets
	@(link_name="ImGui_LabelTextV") LabelTextV        :: proc(label: cstring, fmt: cstring, args: c.va_list) ---
	@(link_name="ImGui_BulletText") BulletText        :: proc(fmt: cstring, #c_vararg _: ..any) ---                 // shortcut for Bullet()+Text()
	@(link_name="ImGui_BulletTextV") BulletTextV       :: proc(fmt: cstring, args: c.va_list) ---
	...
	@(link_name="ImGui_InvisibleButton") InvisibleButton      :: proc(str_id: cstring, size: Vec2, flags: GuiButtonFlags = {}) -> bool --- // flexible button behavior without the visuals, frequently useful to build custom behaviors using the public api (along with IsItemActive, IsItemHovered, etc.)
	@(link_name="ImGui_ArrowButton") ArrowButton          :: proc(str_id: cstring, dir: GuiDir) -> bool ---              // square button with an arrow shape
	...
	@(link_name="ImGui_ProgressBar") ProgressBar          :: proc(fraction: f32, size_arg: Vec2 = {-1.17549435e-38, 0}, overlay: cstring = nil) ---
	...
	@(link_name="ImGui_TextLinkOpenURLEx") TextLinkOpenURL    :: proc(label: cstring, url: cstring = nil) -> bool --- // hyperlink text button, automatically open file/url when clicked
	...
	@(link_name="ImGui_ImageEx") Image       :: proc(tex_ref: TextureRef, image_size: Vec2, uv0: Vec2 = {0, 0}, uv1: Vec2 = {1, 1}) ---
	@(link_name="ImGui_ImageWithBgEx") ImageWithBg :: proc(tex_ref: TextureRef, image_size: Vec2, uv0: Vec2 = {0, 0}, uv1: Vec2 = {1, 1}, bg_col: Vec4 = {0, 0, 0, 0}, tint_col: Vec4 = {1, 1, 1, 1}) ---
	@(link_name="ImGui_ImageButtonEx") ImageButton :: proc(str_id: cstring, tex_ref: TextureRef, image_size: Vec2, uv0: Vec2 = {0, 0}, uv1: Vec2 = {1, 1}, bg_col: Vec4 = {0, 0, 0, 0}, tint_col: Vec4 = {1, 1, 1, 1}) -> bool ---

	// Widgets: Combo Box (Dropdown)
	// - The BeginCombo()/EndCombo() api allows you to manage your contents and selection state however you want it, by creating e.g. Selectable() items.
	// - The old Combo() api are helpers over BeginCombo()/EndCombo() which are kept available for convenience purpose. This is analogous to how ListBox are created.
	@(link_name="ImGui_BeginCombo") BeginCombo      :: proc(label: cstring, preview_value: cstring, flags: GuiComboFlags = {}) -> bool ---
	@(link_name="ImGui_EndCombo") EndCombo        :: proc() --- // only call EndCombo() if BeginCombo() returns true!
	@(link_name="ImGui_ComboCharEx") ComboChar     :: proc(label: cstring, current_item: ^i32, items: [^]cstring, items_count: i32, popup_max_height_in_items: i32 = -1) -> bool ---
	@(link_name="ImGui_ComboEx") Combo         :: proc(label: cstring, current_item: ^i32, items_separated_by_zeros: cstring, popup_max_height_in_items: i32 = -1) -> bool --- // Separate items with \0 within a string, end item-list with \0\0. e.g. "One\0Two\0Three\0"
	@(link_name="ImGui_ComboCallbackEx") ComboCallback :: proc(label: cstring, current_item: ^i32, getter: proc "c" (user_data: rawptr, idx: i32) -> cstring, user_data: rawptr, items_count: i32, popup_max_height_in_items: i32 /* = -1 */) -> bool ---
	...
	@(link_name="ImGui_DragFloatEx") DragFloat       :: proc(label: cstring, v: ^f32, v_speed: f32 = 1, v_min: f32 = 0, v_max: f32 = 0, format: cstring = "%.3f", flags: GuiSliderFlags = {}) -> bool --- // If v_min >= v_max we have no bound
	@(link_name="ImGui_DragFloat2Ex") DragFloat2      :: proc(label: cstring, v: ^[2]f32, v_speed: f32 = 1, v_min: f32 = 0, v_max: f32 = 0, format: cstring = "%.3f", flags: GuiSliderFlags = {}) -> bool ---
	@(link_name="ImGui_DragFloat3Ex") DragFloat3      :: proc(label: cstring, v: ^[3]f32, v_speed: f32 = 1, v_min: f32 = 0, v_max: f32 = 0, format: cstring = "%.3f", flags: GuiSliderFlags = {}) -> bool ---
	@(link_name="ImGui_DragFloat4Ex") DragFloat4      :: proc(label: cstring, v: ^[4]f32, v_speed: f32 = 1, v_min: f32 = 0, v_max: f32 = 0, format: cstring = "%.3f", flags: GuiSliderFlags = {}) -> bool ---
	@(link_name="ImGui_DragFloatRange2Ex") DragFloatRange2 :: proc(label: cstring, v_current_min: ^f32, v_current_max: ^f32, v_speed: f32 = 1, v_min: f32 = 0, v_max: f32 = 0, format: cstring = "%.3f", format_max: cstring = nil, flags: GuiSliderFlags = {}) -> bool ---
	@(link_name="ImGui_DragIntEx") DragInt         :: proc(label: cstring, v: ^i32, v_speed: f32 = 1, v_min: i32 = 0, v_max: i32 = 0, format: cstring = "%d", flags: GuiSliderFlags = {}) -> bool --- // If v_min >= v_max we have no bound
	@(link_name="ImGui_DragInt2Ex") DragInt2        :: proc(label: cstring, v: ^[2]i32, v_speed: f32 = 1, v_min: i32 = 0, v_max: i32 = 0, format: cstring = "%d", flags: GuiSliderFlags = {}) -> bool ---
	@(link_name="ImGui_DragInt3Ex") DragInt3        :: proc(label: cstring, v: ^[3]i32, v_speed: f32 = 1, v_min: i32 = 0, v_max: i32 = 0, format: cstring = "%d", flags: GuiSliderFlags = {}) -> bool ---
	@(link_name="ImGui_DragInt4Ex") DragInt4        :: proc(label: cstring, v: ^[4]i32, v_speed: f32 = 1, v_min: i32 = 0, v_max: i32 = 0, format: cstring = "%d", flags: GuiSliderFlags = {}) -> bool ---
	@(link_name="ImGui_DragIntRange2Ex") DragIntRange2   :: proc(label: cstring, v_current_min: ^i32, v_current_max: ^i32, v_speed: f32 = 1, v_min: i32 = 0, v_max: i32 = 0, format: cstring = "%d", format_max: cstring = nil, flags: GuiSliderFlags = {}) -> bool ---
	@(link_name="ImGui_DragScalarEx") DragScalar      :: proc(label: cstring, data_type: GuiDataType, p_data: rawptr, v_speed: f32 = 1, p_min: rawptr = nil, p_max: rawptr = nil, format: cstring = nil, flags: GuiSliderFlags = {}) -> bool ---
	@(link_name="ImGui_DragScalarNEx") DragScalarN     :: proc(label: cstring, data_type: GuiDataType, p_data: rawptr, components: i32, v_speed: f32 = 1, p_min: rawptr = nil, p_max: rawptr = nil, format: cstring = nil, flags: GuiSliderFlags = {}) -> bool ---
	...
	//   If you get a warning converting a float to ImGuiSliderFlags, read https://github.com/ocornut/imgui/issues/3361
	@(link_name="ImGui_SliderFloatEx") SliderFloat   :: proc(label: cstring, v: ^f32, v_min: f32, v_max: f32, format: cstring = "%.3f", flags: GuiSliderFlags = {}) -> bool --- // adjust format to decorate the value with a prefix or a suffix for in-slider labels or unit display.
	@(link_name="ImGui_SliderFloat2Ex") SliderFloat2  :: proc(label: cstring, v: ^[2]f32, v_min: f32, v_max: f32, format: cstring = "%.3f", flags: GuiSliderFlags = {}) -> bool ---
	@(link_name="ImGui_SliderFloat3Ex") SliderFloat3  :: proc(label: cstring, v: ^[3]f32, v_min: f32, v_max: f32, format: cstring = "%.3f", flags: GuiSliderFlags = {}) -> bool ---
	@(link_name="ImGui_SliderFloat4Ex") SliderFloat4  :: proc(label: cstring, v: ^[4]f32, v_min: f32, v_max: f32, format: cstring = "%.3f", flags: GuiSliderFlags = {}) -> bool ---
	@(link_name="ImGui_SliderAngleEx") SliderAngle   :: proc(label: cstring, v_rad: ^f32, v_degrees_min: f32 = -360, v_degrees_max: f32 = 360, format: cstring = "%.0f deg", flags: GuiSliderFlags = {}) -> bool ---
	@(link_name="ImGui_SliderIntEx") SliderInt     :: proc(label: cstring, v: ^i32, v_min: i32, v_max: i32, format: cstring = "%d", flags: GuiSliderFlags = {}) -> bool ---
	@(link_name="ImGui_SliderInt2Ex") SliderInt2    :: proc(label: cstring, v: ^[2]i32, v_min: i32, v_max: i32, format: cstring = "%d", flags: GuiSliderFlags = {}) -> bool ---
	@(link_name="ImGui_SliderInt3Ex") SliderInt3    :: proc(label: cstring, v: ^[3]i32, v_min: i32, v_max: i32, format: cstring = "%d", flags: GuiSliderFlags = {}) -> bool ---
	@(link_name="ImGui_SliderInt4Ex") SliderInt4    :: proc(label: cstring, v: ^[4]i32, v_min: i32, v_max: i32, format: cstring = "%d", flags: GuiSliderFlags = {}) -> bool ---
	@(link_name="ImGui_SliderScalarEx") SliderScalar  :: proc(label: cstring, data_type: GuiDataType, p_data: rawptr, p_min: rawptr, p_max: rawptr, format: cstring = nil, flags: GuiSliderFlags = {}) -> bool ---
	@(link_name="ImGui_SliderScalarNEx") SliderScalarN :: proc(label: cstring, data_type: GuiDataType, p_data: rawptr, components: i32, p_min: rawptr, p_max: rawptr, format: cstring = nil, flags: GuiSliderFlags = {}) -> bool ---
	@(link_name="ImGui_VSliderFloatEx") VSliderFloat  :: proc(label: cstring, size: Vec2, v: ^f32, v_min: f32, v_max: f32, format: cstring = "%.3f", flags: GuiSliderFlags = {}) -> bool ---
	@(link_name="ImGui_VSliderIntEx") VSliderInt    :: proc(label: cstring, size: Vec2, v: ^i32, v_min: i32, v_max: i32, format: cstring = "%d", flags: GuiSliderFlags = {}) -> bool ---
	@(link_name="ImGui_VSliderScalarEx") VSliderScalar :: proc(label: cstring, size: Vec2, data_type: GuiDataType, p_data: rawptr, p_min: rawptr, p_max: rawptr, format: cstring = nil, flags: GuiSliderFlags = {}) -> bool ---
	...
	@(link_name="ImGui_InputTextEx") InputText          :: proc(label: cstring, buf: cstring, buf_size: c.size_t, flags: GuiInputTextFlags = {}, callback: GuiInputTextCallback = nil, user_data: rawptr = nil) -> bool ---
	@(link_name="ImGui_InputTextMultilineEx") InputTextMultiline :: proc(label: cstring, buf: cstring, buf_size: c.size_t, size: Vec2 = {0, 0}, flags: GuiInputTextFlags = {}, callback: GuiInputTextCallback = nil, user_data: rawptr = nil) -> bool ---
	@(link_name="ImGui_InputTextWithHintEx") InputTextWithHint  :: proc(label: cstring, hint: cstring, buf: cstring, buf_size: c.size_t, flags: GuiInputTextFlags = {}, callback: GuiInputTextCallback = nil, user_data: rawptr = nil) -> bool ---
	@(link_name="ImGui_InputFloatEx") InputFloat         :: proc(label: cstring, v: ^f32, step: f32 = 0, step_fast: f32 = 0, format: cstring = "%.3f", flags: GuiInputTextFlags = {}) -> bool ---
	@(link_name="ImGui_InputFloat2Ex") InputFloat2        :: proc(label: cstring, v: ^[2]f32, format: cstring = "%.3f", flags: GuiInputTextFlags = {}) -> bool ---
	@(link_name="ImGui_InputFloat3Ex") InputFloat3        :: proc(label: cstring, v: ^[3]f32, format: cstring = "%.3f", flags: GuiInputTextFlags = {}) -> bool ---
	@(link_name="ImGui_InputFloat4Ex") InputFloat4        :: proc(label: cstring, v: ^[4]f32, format: cstring = "%.3f", flags: GuiInputTextFlags = {}) -> bool ---
	@(link_name="ImGui_InputIntEx") InputInt           :: proc(label: cstring, v: ^i32, step: i32 = 1, step_fast: i32 = 100, flags: GuiInputTextFlags = {}) -> bool ---
	@(link_name="ImGui_InputInt2") InputInt2            :: proc(label: cstring, v: ^[2]i32, flags: GuiInputTextFlags = {}) -> bool ---
	@(link_name="ImGui_InputInt3") InputInt3            :: proc(label: cstring, v: ^[3]i32, flags: GuiInputTextFlags = {}) -> bool ---
	@(link_name="ImGui_InputInt4") InputInt4            :: proc(label: cstring, v: ^[4]i32, flags: GuiInputTextFlags = {}) -> bool ---
	@(link_name="ImGui_InputDoubleEx") InputDouble        :: proc(label: cstring, v: ^f64, step: f64 = 0, step_fast: f64 = 0, format: cstring = "%.6f", flags: GuiInputTextFlags = {}) -> bool ---
	@(link_name="ImGui_InputScalarEx") InputScalar        :: proc(label: cstring, data_type: GuiDataType, p_data: rawptr, p_step: rawptr = nil, p_step_fast: rawptr = nil, format: cstring = nil, flags: GuiInputTextFlags = {}) -> bool ---
	@(link_name="ImGui_InputScalarNEx") InputScalarN       :: proc(label: cstring, data_type: GuiDataType, p_data: rawptr, components: i32, p_step: rawptr = nil, p_step_fast: rawptr = nil, format: cstring = nil, flags: GuiInputTextFlags = {}) -> bool ---

	// Widgets: Color Editor/Picker (tip: the ColorEdit* functions have a little color square that can be left-clicked to open a picker, and right-clicked to open an option menu.)
	// - Note that in C++ a 'float v[X]' function argument is the _same_ as 'float* v', the array syntax is just a way to document the number of elements that are expected to be accessible.
	// - You can pass the address of a first float element out of a contiguous structure, e.g. &myvector.x
	@(link_name="ImGui_ColorEdit3") ColorEdit3          :: proc(label: cstring, col: ^[3]f32, flags: GuiColorEditFlags = {}) -> bool ---
	@(link_name="ImGui_ColorEdit4") ColorEdit4          :: proc(label: cstring, col: ^[4]f32, flags: GuiColorEditFlags = {}) -> bool ---
	@(link_name="ImGui_ColorPicker3") ColorPicker3        :: proc(label: cstring, col: ^[3]f32, flags: GuiColorEditFlags = {}) -> bool ---
	@(link_name="ImGui_ColorPicker4") ColorPicker4        :: proc(label: cstring, col: ^[4]f32, flags: GuiColorEditFlags = {}, ref_col: ^f32 = nil) -> bool ---
	@(link_name="ImGui_ColorButtonEx") ColorButton       :: proc(desc_id: cstring, col: Vec4, flags: GuiColorEditFlags = {}, size: Vec2 = {0, 0}) -> bool --- // display a color square/button, hover for details, return true when pressed.
	@(link_name="ImGui_SetColorEditOptions") SetColorEditOptions :: proc(flags: GuiColorEditFlags) --- // initialize current options (generally on application startup) if you want to select a default format, picker type, etc. User will be able to change many settings, unless you pass the _NoOptions flag to your calls.

	// Widgets: Trees
	// - TreeNode functions return true when the node is open, in which case you need to also call TreePop() when you are finished displaying the tree node contents.
	@(link_name="ImGui_TreeNode") TreeNode                  :: proc(label: cstring) -> bool ---
	@(link_name="ImGui_TreeNodeStr") TreeNodeStr               :: proc(str_id: cstring, fmt: cstring, #c_vararg _: ..any) -> bool --- // helper variation to easily decorrelate the id from the displayed string. Read the FAQ about why and how to use ID. to align arbitrary text at the same level as a TreeNode() you can use Bullet().
	@(link_name="ImGui_TreeNodePtr") TreeNodePtr               :: proc(ptr_id: rawptr, fmt: cstring, #c_vararg _: ..any) -> bool --- // "
	@(link_name="ImGui_TreeNodeV") TreeNodeV                 :: proc(str_id: cstring, fmt: cstring, args: c.va_list) -> bool ---
	@(link_name="ImGui_TreeNodeVPtr") TreeNodeVPtr              :: proc(ptr_id: rawptr, fmt: cstring, args: c.va_list) -> bool ---
	@(link_name="ImGui_TreeNodeEx") TreeNodeEx                :: proc(label: cstring, flags: GuiTreeNodeFlags = {}) -> bool ---
	@(link_name="ImGui_TreeNodeExStr") TreeNodeExStr             :: proc(str_id: cstring, flags: GuiTreeNodeFlags, fmt: cstring, #c_vararg _: ..any) -> bool ---
	@(link_name="ImGui_TreeNodeExPtr") TreeNodeExPtr             :: proc(ptr_id: rawptr, flags: GuiTreeNodeFlags, fmt: cstring, #c_vararg _: ..any) -> bool ---
	@(link_name="ImGui_TreeNodeExV") TreeNodeExV               :: proc(str_id: cstring, flags: GuiTreeNodeFlags, fmt: cstring, args: c.va_list) -> bool ---
	@(link_name="ImGui_TreeNodeExVPtr") TreeNodeExVPtr            :: proc(ptr_id: rawptr, flags: GuiTreeNodeFlags, fmt: cstring, args: c.va_list) -> bool ---
	...
	@(link_name="ImGui_CollapsingHeader") CollapsingHeader          :: proc(label: cstring, flags: GuiTreeNodeFlags = {}) -> bool --- // if returning 'true' the header is open. doesn't indent nor push on ID stack. user doesn't have to call TreePop().
	@(link_name="ImGui_CollapsingHeaderBoolPtr") CollapsingHeaderBoolPtr   :: proc(label: cstring, p_visible: ^bool, flags: GuiTreeNodeFlags = {}) -> bool --- // when 'p_visible != NULL': if '*p_visible==true' display an additional small close button on upper right of the header which will set the bool to false when clicked, if '*p_visible==false' don't display the header.
	@(link_name="ImGui_SetNextItemOpen") SetNextItemOpen           :: proc(is_open: bool, cond: GuiCond = .None) --- // set next TreeNode/CollapsingHeader open state.
	@(link_name="ImGui_SetNextItemStorageID") SetNextItemStorageID      :: proc(storage_id: GuiID) ---                      // set id to use for open/close storage (default to same as item id).

	// Widgets: Selectables
	// - A selectable highlights when hovered, and can display another color when selected.
	// - Neighbors selectable extend their highlight bounds in order to leave no gap between them. This is so a series of selected Selectable appear contiguous.
	@(link_name="ImGui_SelectableEx") Selectable        :: proc(label: cstring, selected: bool = false, flags: GuiSelectableFlags = {}, size: Vec2 = {0, 0}) -> bool --- // "bool selected" carry the selection state (read-only). Selectable() is clicked is returns true so you can modify your selection state. size.x==0.0: use remaining width, size.x>0.0: specify width. size.y==0.0: use label height, size.y>0.0: specify height
	@(link_name="ImGui_SelectableBoolPtrEx") SelectableBoolPtr :: proc(label: cstring, p_selected: ^bool, flags: GuiSelectableFlags = {}, size: Vec2 = {0, 0}) -> bool --- // "bool* p_selected" point to the selection state (read-write), as a convenient helper.
	...
	@(link_name="ImGui_BeginMultiSelectEx") BeginMultiSelect           :: proc(flags: GuiMultiSelectFlags, selection_size: i32 = -1, items_count: i32 = -1) -> ^GuiMultiSelectIO ---
	@(link_name="ImGui_EndMultiSelect") EndMultiSelect               :: proc() -> ^GuiMultiSelectIO ---
	@(link_name="ImGui_SetNextItemSelectionUserData") SetNextItemSelectionUserData :: proc(selection_user_data: GuiSelectionUserData) ---
	...
	@(link_name="ImGui_ListBoxCallbackEx") ListBoxCallback :: proc(label: cstring, current_item: ^i32, getter: proc "c" (user_data: rawptr, idx: i32) -> cstring, user_data: rawptr, items_count: i32, height_in_items: i32 /* = -1 */) -> bool ---

	// Widgets: Data Plotting
	// - Consider using ImPlot (https://github.com/epezent/implot) which is much better!
	@(link_name="ImGui_PlotLinesEx") PlotLines             :: proc(label: cstring, values: ^f32, values_count: i32, values_offset: i32 = 0, overlay_text: cstring = nil, scale_min: f32 = max(f32), scale_max: f32 = max(f32), graph_size: Vec2 = {0, 0}, stride: i32 = size_of(f32)) ---
	@(link_name="ImGui_PlotLinesCallbackEx") PlotLinesCallback     :: proc(label: cstring, values_getter: proc "c" (data: rawptr, idx: i32) -> f32, data: rawptr, values_count: i32, values_offset: i32 /* = 0 */, overlay_text: cstring /* = NULL */, scale_min: f32 /* = FLT_MAX */, scale_max: f32 /* = FLT_MAX */, graph_size: Vec2 /* = ImVec2(0, 0) */) ---
	@(link_name="ImGui_PlotHistogramEx") PlotHistogram         :: proc(label: cstring, values: ^f32, values_count: i32, values_offset: i32 = 0, overlay_text: cstring = nil, scale_min: f32 = max(f32), scale_max: f32 = max(f32), graph_size: Vec2 = {0, 0}, stride: i32 = size_of(f32)) ---
	@(link_name="ImGui_PlotHistogramCallbackEx") PlotHistogramCallback :: proc(label: cstring, values_getter: proc "c" (data: rawptr, idx: i32) -> f32, data: rawptr, values_count: i32, values_offset: i32 /* = 0 */, overlay_text: cstring /* = NULL */, scale_min: f32 /* = FLT_MAX */, scale_max: f32 /* = FLT_MAX */, graph_size: Vec2 /* = ImVec2(0, 0) */) ---
	...
	@(link_name="ImGui_SetTooltip") SetTooltip   :: proc(fmt: cstring, #c_vararg _: ..any) --- // set a text-only tooltip. Often used after a ImGui::IsItemHovered() check. Override any previous call to SetTooltip().
	@(link_name="ImGui_SetTooltipV") SetTooltipV  :: proc(fmt: cstring, args: c.va_list) ---
	...
	@(link_name="ImGui_SetItemTooltip") SetItemTooltip   :: proc(fmt: cstring, #c_vararg _: ..any) --- // set a text-only tooltip if preceding item was hovered. override any previous call to SetTooltip().
	@(link_name="ImGui_SetItemTooltipV") SetItemTooltipV  :: proc(fmt: cstring, args: c.va_list) ---
	...
	@(link_name="ImGui_BeginPopup") BeginPopup      :: proc(str_id: cstring, flags: GuiWindowFlags = {}) -> bool --- // return true if the popup is open, and you can start outputting to it.
	@(link_name="ImGui_BeginPopupModal") BeginPopupModal :: proc(name: cstring, p_open: ^bool = nil, flags: GuiWindowFlags = {}) -> bool --- // return true if the modal is open, and you can start outputting to it.
	...
	@(link_name="ImGui_OpenPopup") OpenPopup            :: proc(str_id: cstring, popup_flags: GuiPopupFlags = {}) --- // call to mark popup as open (don't call every frame!).
	@(link_name="ImGui_OpenPopupID") OpenPopupID          :: proc(id: GuiID, popup_flags: GuiPopupFlags = {}) --- // id overload to facilitate calling from nested stacks
	@(link_name="ImGui_OpenPopupOnItemClick") OpenPopupOnItemClick :: proc(str_id: cstring = nil, popup_flags: GuiPopupFlags = {}) --- // helper to open popup when clicked on last item. Default to ImGuiPopupFlags_MouseButtonRight == 1. (note: actually triggers on the mouse _released_ event to be consistent with popup behaviors)
	...
	@(link_name="ImGui_BeginPopupContextItemEx") BeginPopupContextItem   :: proc(str_id: cstring = nil, popup_flags: GuiPopupFlags = {}) -> bool --- // open+begin popup when clicked on last item. Use str_id==NULL to associate the popup to previous item. If you want to use that on a non-interactive item such as Text() you need to pass in an explicit ID here. read comments in .cpp!
	@(link_name="ImGui_BeginPopupContextWindowEx") BeginPopupContextWindow :: proc(str_id: cstring = nil, popup_flags: GuiPopupFlags = {}) -> bool --- // open+begin popup when clicked on current window.
	@(link_name="ImGui_BeginPopupContextVoidEx") BeginPopupContextVoid   :: proc(str_id: cstring = nil, popup_flags: GuiPopupFlags = {}) -> bool --- // open+begin popup when clicked in void (where there are no windows).

	// Popups: query functions
	//  - IsPopupOpen(): return true if the popup is open at the current BeginPopup() level of the popup stack.
	//  - IsPopupOpen() with ImGuiPopupFlags_AnyPopupId: return true if any popup is open at the current BeginPopup() level of the popup stack.
	//  - IsPopupOpen() with ImGuiPopupFlags_AnyPopupId + ImGuiPopupFlags_AnyPopupLevel: return true if any popup is open.
	@(link_name="ImGui_IsPopupOpen") IsPopupOpen :: proc(str_id: cstring, flags: GuiPopupFlags = {}) -> bool --- // return true if the popup is open.
	...
	// - 5. Call EndTable()
	@(link_name="ImGui_BeginTableEx") BeginTable        :: proc(str_id: cstring, columns: i32, flags: GuiTableFlags = {}, outer_size: Vec2 = {0, 0}, inner_width: f32 = 0) -> bool ---
	@(link_name="ImGui_EndTable") EndTable            :: proc() ---                      // only call EndTable() if BeginTable() returns true!
	@(link_name="ImGui_TableNextRowEx") TableNextRow      :: proc(row_flags: GuiTableRowFlags = {}, min_row_height: f32 = 0) --- // append into the first cell of a new row. 'min_row_height' include the minimum top and bottom padding aka CellPadding.y * 2.0f.
	...
	@(link_name="ImGui_TableSetupColumnEx") TableSetupColumn     :: proc(label: cstring, flags: GuiTableColumnFlags = {}, init_width_or_weight: f32 = 0, user_id: GuiID = 0) ---
	...
	@(link_name="ImGui_TableGetSortSpecs") TableGetSortSpecs     :: proc() -> ^GuiTableSortSpecs ---                          // get latest sort specs for the table (NULL if not sorting).  Lifetime: don't hold on this pointer over multiple frames or past any subsequent call to BeginTable().
	...
	@(link_name="ImGui_TableGetColumnFlags") TableGetColumnFlags   :: proc(column_n: i32 = -1) -> GuiTableColumnFlags --- // return column flags so you can query their Enabled/Visible/Sorted/Hovered status flags. Pass -1 to use current column.
	...
	@(link_name="ImGui_TableSetBgColor") TableSetBgColor       :: proc(target: GuiTableBgTarget, color: u32, column_n: i32 = -1) --- // change the color of a cell, row, or column. See ImGuiTableBgTarget_ flags for details.

	// Legacy Columns API (prefer using Tables!)
	// - You can also use SameLine(pos_x) to mimic simplified columns.
	@(link_name="ImGui_ColumnsEx") Columns       :: proc(count: i32 = 1, id: cstring = nil, borders: bool = true) ---
	...
	@(link_name="ImGui_BeginTabBar") BeginTabBar      :: proc(str_id: cstring, flags: GuiTabBarFlags = {}) -> bool --- // create and append into a TabBar
	@(link_name="ImGui_EndTabBar") EndTabBar        :: proc() ---                                    // only call EndTabBar() if BeginTabBar() returns true!
	@(link_name="ImGui_BeginTabItem") BeginTabItem     :: proc(label: cstring, p_open: ^bool = nil, flags: GuiTabItemFlags = {}) -> bool --- // create a Tab. Returns true if the Tab is selected.
	@(link_name="ImGui_EndTabItem") EndTabItem       :: proc() ---                                    // only call EndTabItem() if BeginTabItem() returns true!
	@(link_name="ImGui_TabItemButton") TabItemButton    :: proc(label: cstring, flags: GuiTabItemFlags = {}) -> bool --- // create a Tab behaving like a button. return true when clicked. cannot be selected in the tab bar.
	...
	@(link_name="ImGui_DockSpaceEx") DockSpace             :: proc(dockspace_id: GuiID, size: Vec2 = {0, 0}, flags: GuiDockNodeFlags = {}, window_class: ^GuiWindowClass = nil) -> GuiID ---
	@(link_name="ImGui_DockSpaceOverViewportEx") DockSpaceOverViewport :: proc(dockspace_id: GuiID = 0, viewport: ^GuiViewport = nil, flags: GuiDockNodeFlags = {}, window_class: ^GuiWindowClass = nil) -> GuiID ---
	@(link_name="ImGui_SetNextWindowDockID") SetNextWindowDockID     :: proc(dock_id: GuiID, cond: GuiCond = .None) --- // set next window dock id
	@(link_name="ImGui_SetNextWindowClass") SetNextWindowClass      :: proc(window_class: ^GuiWindowClass) ---           // set next window class (control docking compatibility + provide hints to platform backend via custom viewport flags and platform parent/child relationship)
	@(link_name="ImGui_GetWindowDockID") GetWindowDockID         :: proc() -> GuiID ---                               // get dock id of current window, or 0 if not associated to any docking node.
	...
	@(link_name="ImGui_LogText") LogText        :: proc(fmt: cstring, #c_vararg _: ..any) --- // pass text data straight to log (without being displayed)
	@(link_name="ImGui_LogTextV") LogTextV       :: proc(fmt: cstring, args: c.va_list) ---
	...
	@(link_name="ImGui_BeginDragDropSource") BeginDragDropSource   :: proc(flags: GuiDragDropFlags = {}) -> bool --- // call after submitting an item which may be dragged. when this return true, you can call SetDragDropPayload() + EndDragDropSource()
	@(link_name="ImGui_SetDragDropPayload") SetDragDropPayload    :: proc(type: cstring, data: rawptr, sz: c.size_t, cond: GuiCond = .None) -> bool --- // type is a user defined string of maximum 32 characters. Strings starting with '_' are reserved for dear imgui internal types. Data is copied and held by imgui. Return true when payload has been accepted.
	...
	@(link_name="ImGui_AcceptDragDropPayload") AcceptDragDropPayload :: proc(type: cstring, flags: GuiDragDropFlags = {}) -> ^GuiPayload --- // accept contents of a given type. If ImGuiDragDropFlags_AcceptBeforeDelivery is set you can peek into the payload before the mouse button is released.
	@(link_name="ImGui_EndDragDropTarget") EndDragDropTarget     :: proc() ---                                          // only call EndDragDropTarget() if BeginDragDropTarget() returns true!
	@(link_name="ImGui_GetDragDropPayload") GetDragDropPayload    :: proc() -> ^GuiPayload ---                           // peek directly into the current payload from anywhere. returns NULL when drag and drop is finished or inactive. use ImGuiPayload::IsDataType() to test for the payload type.
	...
	@(link_name="ImGui_SetKeyboardFocusHereEx") SetKeyboardFocusHere :: proc(offset: i32 = 0) --- // focus keyboard on the next widget. Use positive 'offset' to access sub components of a multiple component widget. Use -1 to access previous widget.
	...
	@(link_name="ImGui_IsItemHovered") IsItemHovered              :: proc(flags: GuiHoveredFlags = {}) -> bool --- // is the last item hovered? (and usable, aka not blocked by a popup, etc.). See ImGuiHoveredFlags for more options.
	...
	@(link_name="ImGui_IsItemClickedEx") IsItemClicked            :: proc(mouse_button: GuiMouseButton = .Left) -> bool --- // is the last item hovered and mouse clicked on? (**)  == IsMouseClicked(mouse_button) && IsItemHovered()Important. (**) this is NOT equivalent to the behavior of e.g. Button(). Read comments in function definition.
	...
	@(link_name="ImGui_GetItemID") GetItemID                  :: proc() -> GuiID ---                                // get ID of last item (~~ often same ImGui::GetID(label) beforehand)
	...
	@(link_name="ImGui_GetItemFlags") GetItemFlags               :: proc() -> GuiItemFlags ---                         // get generic flags of last item

	// Viewports
	// - Currently represents the Platform Window created by the application which is hosting our Dear ImGui windows.
	// - In 'docking' branch with multi-viewport enabled, we extend this concept to have multiple active viewports.
	// - In the future we will extend this concept further to also represent Platform Monitor and support a "no main platform window" operation mode.
	@(link_name="ImGui_GetMainViewport") GetMainViewport :: proc() -> ^GuiViewport --- // return primary/default viewport. This can never be NULL.

	// Background/Foreground Draw Lists
	@(link_name="ImGui_GetBackgroundDrawListEx") GetBackgroundDrawList :: proc(viewport: ^GuiViewport = nil) -> ^DrawList --- // get background draw list for the given viewport or viewport associated to the current window. this draw list will be the first rendering one. Useful to quickly draw shapes/text behind dear imgui contents.
	@(link_name="ImGui_GetForegroundDrawListEx") GetForegroundDrawList :: proc(viewport: ^GuiViewport = nil) -> ^DrawList --- // get foreground draw list for the given viewport or viewport associated to the current window. this draw list will be the top-most rendered one. Useful to quickly draw shapes/text over dear imgui contents.
	...
	@(link_name="ImGui_GetStyleColorName") GetStyleColorName     :: proc(idx: GuiCol) -> cstring ---                 // get a string corresponding to the enum value (for display, saving, etc.).
	@(link_name="ImGui_SetStateStorage") SetStateStorage       :: proc(storage: ^GuiStorage) ---                   // replace current window storage with our own (if you want to manipulate it yourself, typically clear subsection of it)
	@(link_name="ImGui_GetStateStorage") GetStateStorage       :: proc() -> ^GuiStorage ---

	// Text Utilities
	@(link_name="ImGui_CalcTextSizeEx") CalcTextSize :: proc(text: cstring, text_end: cstring = nil, hide_text_after_double_hash: bool = false, wrap_width: f32 = -1) -> Vec2 ---
	...
	@(link_name="ImGui_IsKeyDown") IsKeyDown                       :: proc(key: GuiKey) -> bool ---            // is key being held.
	@(link_name="ImGui_IsKeyPressedEx") IsKeyPressed                  :: proc(key: GuiKey, repeat: bool = true) -> bool --- // was key pressed (went from !Down to Down)? Repeat rate uses io.KeyRepeatDelay / KeyRepeatRate.
	@(link_name="ImGui_IsKeyReleased") IsKeyReleased                   :: proc(key: GuiKey) -> bool ---            // was key released (went from Down to !Down)?
	@(link_name="ImGui_IsKeyChordPressed") IsKeyChordPressed               :: proc(key_chord: GuiKeyChord) -> bool --- // was key chord (mods + key) pressed, e.g. you can pass 'ImGuiMod_Ctrl | ImGuiKey_S' as a key-chord. This doesn't do any routing or focus check, please consider using Shortcut() function instead.
	@(link_name="ImGui_GetKeyPressedAmount") GetKeyPressedAmount             :: proc(key: GuiKey, repeat_delay: f32, rate: f32) -> i32 --- // uses provided repeat rate/delay. return a count, most often 0 or 1 but might be >1 if RepeatRate is small enough that DeltaTime > RepeatRate
	@(link_name="ImGui_GetKeyName") GetKeyName                      :: proc(key: GuiKey) -> cstring ---         // [DEBUG] returns English name of the key. Those names are provided for debugging purpose and are not meant to be saved persistently nor compared.
	...
	@(link_name="ImGui_Shortcut") Shortcut            :: proc(key_chord: GuiKeyChord, flags: GuiInputFlags = {}) -> bool ---
	@(link_name="ImGui_SetNextItemShortcut") SetNextItemShortcut :: proc(key_chord: GuiKeyChord, flags: GuiInputFlags = {}) ---
	...
	@(link_name="ImGui_SetItemKeyOwner") SetItemKeyOwner :: proc(key: GuiKey) --- // Set key owner to last item ID if it is hovered or active. Equivalent to 'if (IsItemHovered() || IsItemActive()) { SetKeyOwner(key, GetItemID());'.

	// Inputs Utilities: Mouse
	// - To refer to a mouse button, you may use named enums in your code e.g. ImGuiMouseButton_Left, ImGuiMouseButton_Right.
	// - You can also use regular integer: it is forever guaranteed that 0=Left, 1=Right, 2=Middle.
	// - Dragging operations are only reported after mouse has moved a certain distance away from the initial clicking position (see 'lock_threshold' and 'io.MouseDraggingThreshold')
	@(link_name="ImGui_IsMouseDown") IsMouseDown                      :: proc(button: GuiMouseButton) -> bool ---        // is mouse button held?
	@(link_name="ImGui_IsMouseClickedEx") IsMouseClicked                 :: proc(button: GuiMouseButton, repeat: bool = false) -> bool --- // did mouse button clicked? (went from !Down to Down). Same as GetMouseClickedCount() == 1.
	@(link_name="ImGui_IsMouseReleased") IsMouseReleased                  :: proc(button: GuiMouseButton) -> bool ---        // did mouse button released? (went from Down to !Down)
	@(link_name="ImGui_IsMouseDoubleClicked") IsMouseDoubleClicked             :: proc(button: GuiMouseButton) -> bool ---        // did mouse button double-clicked? Same as GetMouseClickedCount() == 2. (note that a double-click will also report IsMouseClicked() == true)
	@(link_name="ImGui_IsMouseReleasedWithDelay") IsMouseReleasedWithDelay         :: proc(button: GuiMouseButton, delay: f32) -> bool --- // delayed mouse release (use very sparingly!). Generally used with 'delay >= io.MouseDoubleClickTime' + combined with a 'io.MouseClickedLastCount==1' test. This is a very rarely used UI idiom, but some apps use this: e.g. MS Explorer single click on an icon to rename.
	@(link_name="ImGui_GetMouseClickedCount") GetMouseClickedCount             :: proc(button: GuiMouseButton) -> i32 ---         // return the number of successive mouse-clicks at the time where a click happen (otherwise 0).
	...
	@(link_name="ImGui_IsMouseDragging") IsMouseDragging                  :: proc(button: GuiMouseButton, lock_threshold: f32 = -1) -> bool --- // is mouse dragging? (uses io.MouseDraggingThreshold if lock_threshold < 0.0f)
	@(link_name="ImGui_GetMouseDragDelta") GetMouseDragDelta                :: proc(button: GuiMouseButton = .Left, lock_threshold: f32 = -1) -> Vec2 --- // return the delta from the initial clicking position while the mouse button is pressed or was just released. This is locked and return 0.0f until the mouse moves past a distance threshold at least once (uses io.MouseDraggingThreshold if lock_threshold < 0.0f)
	@(link_name="ImGui_ResetMouseDragDeltaEx") ResetMouseDragDelta            :: proc(button: GuiMouseButton = .Left) ---      //
	@(link_name="ImGui_GetMouseCursor") GetMouseCursor                   :: proc() -> GuiMouseCursor ---                    // get desired mouse cursor shape. Important: reset in ImGui::NewFrame(), this is updated during the frame. valid before Render(). If you use software rendering by setting io.MouseDrawCursor ImGui will render those for you
	@(link_name="ImGui_SetMouseCursor") SetMouseCursor                   :: proc(cursor_type: GuiMouseCursor) ---           // set desired mouse cursor shape
	...
	@(link_name="ImGui_LoadIniSettingsFromMemory") LoadIniSettingsFromMemory :: proc(ini_data: cstring, ini_size: c.size_t = 0) --- // call after CreateContext() and before the first call to NewFrame() to provide .ini data from your own data source.
	...
	@(link_name="ImGui_DebugFlashStyleColor") DebugFlashStyleColor           :: proc(idx: GuiCol) ---
	...
	@(link_name="ImGui_DebugLog") DebugLog                       :: proc(fmt: cstring, #c_vararg _: ..any) --- // Call via IMGUI_DEBUG_LOG() for maximum stripping in caller code!
	@(link_name="ImGui_DebugLogV") DebugLogV                      :: proc(fmt: cstring, args: c.va_list) ---

	// Memory Allocators
	// - Those functions are not reliant on the current context.
	// - DLL users: heaps and globals are not shared across DLL boundaries! You will need to call SetCurrentContext() + SetAllocatorFunctions()
	//   for each static/DLL boundary you are calling from. Read "Context and Memory Allocators" section of imgui.cpp for more details.
	@(link_name="ImGui_SetAllocatorFunctions") SetAllocatorFunctions :: proc(alloc_func: GuiMemAllocFunc, free_func: GuiMemFreeFunc, user_data: rawptr = nil) ---
	@(link_name="ImGui_GetAllocatorFunctions") GetAllocatorFunctions :: proc(p_alloc_func: ^GuiMemAllocFunc, p_free_func: ^GuiMemFreeFunc, p_user_data: ^rawptr) ---
	...
	@(link_name="ImGui_FindViewportByID") FindViewportByID               :: proc(viewport_id: GuiID) -> ^GuiViewport ---      // this is a helper for backends.
	@(link_name="ImGui_FindViewportByPlatformHandle") FindViewportByPlatformHandle   :: proc(platform_handle: rawptr) -> ^GuiViewport --- // this is a helper for backends. the type platform_handle is decided by the backend (e.g. HWND, MyWindow*, GLFWwindow* etc.)
	...
	GuiIO_SetKeyEventNativeData           :: proc(self: ^GuiIO, key: GuiKey, native_keycode: i32, native_scancode: i32, native_legacy_index: i32 = -1) --- // [Optional] Specify index for legacy <1.87 IsKeyXXX() functions with native indices + specify native keycode, scancode.
	...
	GuiInputTextCallbackData_InsertChars    :: proc(self: ^GuiInputTextCallbackData, pos: i32, text: cstring, text_end: cstring = nil) ---
	...
	GuiTextFilter_Draw                      :: proc(self: ^GuiTextFilter, label: cstring = "Filter (inc,-exc)", width: f32 = 0) -> bool --- // Helper calling InputText+Build
	GuiTextFilter_PassFilter                :: proc(self: ^GuiTextFilter, text: cstring, text_end: cstring = nil) -> bool ---
	...
	GuiTextBuffer_append                    :: proc(self: ^GuiTextBuffer, str: cstring, str_end: cstring = nil) ---
	...
	GuiStorage_GetFloat   :: proc(self: ^GuiStorage, key: GuiID, default_val: f32 = 0) -> f32 ---
	...
	GuiStorage_GetFloatRef   :: proc(self: ^GuiStorage, key: GuiID, default_val: f32 = 0) -> ^f32 ---
	GuiStorage_GetVoidPtrRef :: proc(self: ^GuiStorage, key: GuiID, default_val: rawptr = nil) -> ^rawptr ---
	...
	GuiListClipper_Begin :: proc(self: ^GuiListClipper, items_count: i32, items_height: f32 = -1) ---
	...
	Color_SetHSV                                   :: proc(self: ^Color, h: f32, s: f32, v: f32, a: f32 = 1) ---
	Color_HSV                                      :: proc(h: f32, s: f32, v: f32, a: f32 = 1) -> Color ---
	...
	DrawList_AddLine               :: proc(self: ^DrawList, p1: Vec2, p2: Vec2, col: u32, thickness: f32 = 1) ---
	DrawList_AddRect               :: proc(self: ^DrawList, p_min: Vec2, p_max: Vec2, col: u32, rounding: f32 = 0, flags: DrawFlags = {}, thickness: f32 = 1) --- // a: upper-left, b: lower-right (== upper-left + size)
	DrawList_AddRectFilled         :: proc(self: ^DrawList, p_min: Vec2, p_max: Vec2, col: u32, rounding: f32 = 0, flags: DrawFlags = {}) --- // a: upper-left, b: lower-right (== upper-left + size)
	...
	DrawList_AddQuad               :: proc(self: ^DrawList, p1: Vec2, p2: Vec2, p3: Vec2, p4: Vec2, col: u32, thickness: f32 = 1) ---
	DrawList_AddQuadFilled           :: proc(self: ^DrawList, p1: Vec2, p2: Vec2, p3: Vec2, p4: Vec2, col: u32) ---
	DrawList_AddTriangle           :: proc(self: ^DrawList, p1: Vec2, p2: Vec2, p3: Vec2, col: u32, thickness: f32 = 1) ---
	DrawList_AddTriangleFilled       :: proc(self: ^DrawList, p1: Vec2, p2: Vec2, p3: Vec2, col: u32) ---
	DrawList_AddCircle             :: proc(self: ^DrawList, center: Vec2, radius: f32, col: u32, num_segments: i32 = 0, thickness: f32 = 1) ---
	DrawList_AddCircleFilled         :: proc(self: ^DrawList, center: Vec2, radius: f32, col: u32, num_segments: i32 = 0) ---
	DrawList_AddNgon               :: proc(self: ^DrawList, center: Vec2, radius: f32, col: u32, num_segments: i32, thickness: f32 = 1) ---
	DrawList_AddNgonFilled           :: proc(self: ^DrawList, center: Vec2, radius: f32, col: u32, num_segments: i32) ---
	DrawList_AddEllipse            :: proc(self: ^DrawList, center: Vec2, radius: Vec2, col: u32, rot: f32 = 0, num_segments: i32 = 0, thickness: f32 = 1) ---
	DrawList_AddEllipseFilled      :: proc(self: ^DrawList, center: Vec2, radius: Vec2, col: u32, rot: f32 = 0, num_segments: i32 = 0) ---
	DrawList_AddText               :: proc(self: ^DrawList, pos: Vec2, col: u32, text_begin: cstring, text_end: cstring = nil) ---
	DrawList_AddTextImFontPtr      :: proc(self: ^DrawList, font: ^Font, font_size: f32, pos: Vec2, col: u32, text_begin: cstring, text_end: cstring = nil, wrap_width: f32 = 0, cpu_fine_clip_rect: ^Vec4 = nil) ---
	...
	DrawList_AddImage      :: proc(self: ^DrawList, tex_ref: TextureRef, p_min: Vec2, p_max: Vec2, uv_min: Vec2 = {0, 0}, uv_max: Vec2 = {1, 1}, col: u32 = 0xFFFFFFFF) ---
	DrawList_AddImageQuad  :: proc(self: ^DrawList, tex_ref: TextureRef, p1: Vec2, p2: Vec2, p3: Vec2, p4: Vec2, uv1: Vec2 = {0, 0}, uv2: Vec2 = {1, 0}, uv3: Vec2 = {1, 1}, uv4: Vec2 = {0, 1}, col: u32 = 0xFFFFFFFF) ---
	...
	DrawList_PathStroke                 :: proc(self: ^DrawList, col: u32, flags: DrawFlags = {}, thickness: f32 = 1) ---
	...
	DrawList_PathRect                   :: proc(self: ^DrawList, rect_min: Vec2, rect_max: Vec2, rounding: f32 = 0, flags: DrawFlags = {}) ---
	...
	DrawList_AddCallback :: proc(self: ^DrawList, callback: DrawCallback, userdata: rawptr, userdata_size: c.size_t = 0) ---
	...
	FontAtlas_AddFontFromFileTTF                   :: proc(self: ^FontAtlas, filename: cstring, size_pixels: f32 = 0, font_cfg: ^FontConfig = nil, glyph_ranges: ^Wchar = nil) -> ^Font ---
	FontAtlas_AddFontFromMemoryTTF                 :: proc(self: ^FontAtlas, font_data: rawptr, font_data_size: i32, size_pixels: f32 = 0, font_cfg: ^FontConfig = nil, glyph_ranges: ^Wchar = nil) -> ^Font --- // Note: Transfer ownership of 'ttf_data' to ImFontAtlas! Will be deleted after destruction of the atlas. Set font_cfg->FontDataOwnedByAtlas=false to keep ownership of your data and it won't be freed.
	FontAtlas_AddFontFromMemoryCompressedTTF       :: proc(self: ^FontAtlas, compressed_font_data: rawptr, compressed_font_data_size: i32, size_pixels: f32 = 0, font_cfg: ^FontConfig = nil, glyph_ranges: ^Wchar = nil) -> ^Font --- // 'compressed_font_data' still owned by caller. Compress with binary_to_compressed_c.cpp.
	FontAtlas_AddFontFromMemoryCompressedBase85TTF :: proc(self: ^FontAtlas, compressed_font_data_base85: cstring, size_pixels: f32 = 0, font_cfg: ^FontConfig = nil, glyph_ranges: ^Wchar = nil) -> ^Font --- // 'compressed_font_data_base85' still owned by caller. Compress with binary_to_compressed_c.cpp with -base85 parameter.
	...
	Font_GetFontBaked       :: proc(self: ^Font, font_size: f32, density: f32 = -1) -> ^FontBaked --- // Get or create baked data for given size
	...
	Font_RenderText           :: proc(self: ^Font, draw_list: ^DrawList, size: f32, pos: Vec2, col: u32, clip_rect: Vec4, text_begin: cstring, text_end: cstring, wrap_width: f32 = 0, flags: DrawTextFlags = .None) ---
